...
namespace SMS.DAL
{
    public class Dal_DeptWp
    {
        ...
        {
            return SqlHelper.ExecuteSql(" select dep_no, name, up_dep_no, istop, " +
	            " n_man, convert(varchar(100), n_dd, 111) as n_dd," +
	            " e_man, convert(varchar(100), e_dd, 111) as e_dd" +
            " from DeptWp");
        }
        ...
        public DataTable GetData(string Add_SqlWhere)
        {
            return SqlHelper.ExecuteSql(" select dep_no, name, up_dep_no, istop,  " +
	            " n_man, convert(varchar(100), n_dd, 111) as n_dd, " +
                " e_man, convert(varchar(100), e_dd, 111) as e_dd " + 
                " from DeptWp where 1=1 " + Add_SqlWhere);
        }
        /// <summary>
        /// 查看对应部门数量
        /// </summary>
        ...
        {
            DataTable dt = SqlHelper.ExecuteSql("select dep_no, name, up_dep_no, istop, "+
	            " n_man, convert(varchar(100), n_dd, 111) as n_dd, " +
	            " e_man, convert(varchar(100), e_dd, 111) as e_dd from DeptWp where up_dep_no='" + up_no + "'");
        ...
        //    strSql.Append("select count(1) from DeptWp");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into DeptWp(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update DeptWp set ");
        ...
        public bool Delete(string dep_no)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from DeptWp ");
            strSql.Append(" where dep_no='" + dep_no + "' ");
        ...
        public DataTable GetListByPage(string strWhere, string orderby)
        {
            ...
            strSql.Append(")AS Row, T.dep_no, T.name, T.up_dep_no, T.istop,  ");
	        strSql.Append(" T.n_man, convert(varchar(100), T.n_dd, 111) as n_dd, ");
            strSql.Append(" T.e_man, convert(varchar(100), T.e_dd, 111) as e_dd from DeptWp T ");
